...
public class ServerRemoveRelevantEffect extends WorldStateModifyingPackets {
    private final EntityEffect effect;

    public ServerRemoveRelevantEffect(EntityEffect effect) {
        super(PacketType.SERVER_REMOVE_RELEVANT_EFFECT);
        this.effect = effect;
    }

    public ServerRemoveRelevantEffect(InputBitUtility inputBitUtility, WorldGameState worldGameState) throws IOException {
    ...
    public void executeModification(WorldGameState worldGameState) {
        ((ClientWorldGameState)worldGameState).getReliventEffects().remove(effect);
    }
